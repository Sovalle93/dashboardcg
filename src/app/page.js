// app/page.js - Versión corregida (solo un useHistorial)
"use client";
...
import { BarChart2, RefreshCw, Upload, RotateCcw } from "lucide-react";
  ...
  const renderContenido = () => {
    switch(tabActivo) {
      ...
      case "tickets":
        return (
          <div className="grid-2">
            <TicketPromedio datosNegocio={datosCalculados.datosTicketNegocio} datosTipo={datosCalculados.datosTicketTipo} formatCLP={formatCLP} />
          </div>
        );
      case "avanzado":
        return (
...
export default function Home() {
  ...
  // ✅ UN SOLO useHistorial - en el nivel superior
  const { 
    historial, 
    agregarRegistro, 
    eliminarRegistro, 
  ...
  async function handleArchivos(e) {
    ...
    try {
      ...
      // Guardar en historial (evita duplicados automáticamente)
  ...
  const handleResetTool = () => {
    if (confirm("⚠️ ¿Resetear la herramienta?\n\nEsta acción eliminará todos los datos actuales y volverás al inicio. El historial de cargas se mantendrá.")) {
      setPedidos([]);
      setArchivos([]);
      setError(null);
    }
      ...
      <div style={{ minHeight: "100vh", background: "#f8f8f8" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", padding: "80px 24px" }}>
          <EmptyState 
            ...
            icon="upload"
            actionText="Seleccionar archivos Excel"
            onAction={() => document.getElementById('file-input')?.click()}
          />
          <input id="file-input" type="file" accept=".xls,.xlsx" multiple style={{ display: "none" }} onChange={handleArchivos} />
        </div>
        {error && <p style={{ textAlign: "center", color: "#e53e3e", fontSize: 13 }}>{error}</p>}
      </div>
    );
  }
